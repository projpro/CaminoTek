    ...
    if (storeId === null || storeId === "" || storeId === "0") {
       ...
       // console.log("StoreId: 222")
        if (localStorage.getItem("AppRefreshTimeInterval") != null) {
            appRefreshInterval = window.localStorage.getItem("AppRefreshTimeInterval").trim();
        }
...
function CarryoutOrdersList(status, carryoutpagesize, carryoutcurrentPage,divId) {
    ...
    //console.log("pagesize: " + carryoutpagesize);
    //console.log("currentPage: " + carryoutcurrentPage);
    $("#dvList").html("");
    // $("#dvItem").html("");
    ...
    if (Number(storeId) > 0) {
       // console.log("storeIdq1: " + storeId);
        ...
        if (status.toLowerCase().trim() == "new") {
            ...
            $("#imgNewOrder").attr("src", "images/White/new-white.png");
            $("#imgPendingOrder").attr("src", "images/Pending-Orders.png");
            $("#imgAllOrders").attr("src", "images/All-Orders.png");
            ...
            $("#dvAll").removeAttr("class");
            $("#imgNewOrder").attr("src", "images/new.png");
            $("#imgPendingOrder").attr("src", "images/White/Pending-Orders-white.png");
            $("#imgAllOrders").attr("src", "images/All-Orders.png");
        }
        else {
            ...
            $("#imgNewOrder").attr("src", "images/new.png");
            $("#imgPendingOrder").attr("src", "images/Pending-Orders.png");
            $("#imgAllOrders").attr("src", "images/White/All-Orders-white.png");
                ...
                if (JSON.parse(data).indexOf("No order(s) found") < 0) {
                    ...
                    $.each(JSON.parse(data), function (index, value) {
                       // console.log(data)
                        ...
                        /*------------------Order Row-----------------------*/
                        var html = "<div class=\"order-list\" id='li_" + value.ID + "'>";
                        ...
                        if (value.NOOFITEMS == 1)
                            ...
                            html += "<div>" + value.NOOFITEMS + " items</div>";
                        if (paymentMethod == "Cash On Delivery") {
                            html += "<div class=\"customer-name\">Due on Pickup</div>";
                        }
                        else {
                            if (cardNumber != "") {
                                $("#lblPaymentValue").html("<strong>PAID</strong>, CC ending in " + cardNumber);
                                html += "<div class=\"customer-name\">PAID</div>";
                                html += "<span class=\"cc-number\"> CC " + cardNumber + "</span>";
                            }
                            else {
                                html += "<div class=\"customer-name\">PAID</div>";
                            }
                    ...
                    //    $("#hdnSelectedOrderPickUpTime").val("");
                    ...
                    //                if (phone != "")
                    //                    $("#iconPhone").show();
                    //                $("#lblCutomerName").text(firstName + " " + lastName);
                    ...
                    //                $("#lblPaymentValue").text("");
                    //                if (paymentMethod == "Cash On Delivery") {
                    //                    $("#lblPaymentValue").html("<strong>Due on Pickup</strong>");
                    //                }
                    //                else {
                    //                    if (cardNumber != "") {
                    //                        $("#lblPaymentValue").html("<strong>PAID</strong>, CC ending in " + cardNumber);
                    //                    }
                    //                    else {
                    //                        $("#lblPaymentValue").html("<strong>PAID</strong>");
                    //                    }
                    //                }
                    ...
                    //                htmlDiscount += " <tr>";
                    //                htmlDiscount += "<td colspan=\"3\" style=\"text-align:right; font-weight: bold;\">Coupon (" + value.COUPONCODE + ") Discount:</td>";
                    ...
                    //                        //console.log(value.NOTES);
                    //                        //if (value.NOTES.indexOf("Notes:") > -1) {
                    //                        //    html += "<tr><td colspan='4' style='padding:0 0 5px 5px'><i> " + value.NOTES.replace("<strong>","").replace("</strong>","").replace("<i>","").replace("</i>","") + "</i> </td></tr>";
                    //                        //}
                    //                        //else {
                    //                        var arrNotes = [];
                    ...
                    //                                }
                    //                                else {
                    //                                    //var notesValue = arrNotes[i].replace("<strong>","<br><strong>");
                    //                                    html += "<tr><td colspan='4' style='padding:0 0 0 5px'> <i>" + notesValue.replace("</strong>", "") + "</i> </td></tr>";
                    //                                }
                    //                            }
                    //                        }
                    ...
                    //                        //html += "<tr><td colspan='4' style='padding:0 0 5px 5px'> " + value.NOTES.replace("Special Instructions", "Notes") + " </td></tr>";
            ...
            $.getJSON(url, function (data) {
                ...
                if (JSON.parse(data).indexOf("No order(s) found") < 0) {
                    ...
                    $('ul.nav-list li').click(function () {
                        ...
                        var ordertotalvalue = "0.00";
                        var orderDiscount = 0.00;
                        var orderId = id.toString().split('_')[1];
                        var paymentMethod = "";
                        var cardNumber = "";
                        // console.log("Pagination orderId; " + orderId)
                        url = global + "/GetCarryOutOrderDetailsWithAllInfo?orderid=" + orderId;
                        $.getJSON(url, function (data) {
                                ...
                                if (data.indexOf("No record(s) found.") > -1) {
                                    ...
                                    $.each(JSON.parse(data), function (index, value) {
                                        if (value.NOTES != "") {
                                            ...
                                            //}
                                            //html += "<tr><td colspan='4'> " + value.NOTES.replace("Special Instructions", "Notes") + " </td></tr>";
                                        }
                                        else {
                                            html += "<tr><td style='font-weight:bold;'>" + value.PRODUCT + "</td>";
                                            html += "<td style=\"text-align:center;\">" + value.QUANTITY + "</td>";
        ...
        $.getJSON(url, function (data) {
            $.each(JSON.parse(data), function (index, value) {
                ...
                if (carryoutcurrentstatus.toLowerCase().trim() == "running") {
                    $("#dvCarryOutStatusChange").html("<a style=\"color:#e80000;border-color:#e80000;\" class=\"start-btn-one\" onclick=\"ChangeCarryoutStatus(" + storeId + ",'STOPPED')\"><i class=\"fa fa-stop start-font\" aria-hidden=\"true\"></i>STOP</a>");
                }
                else {
                    $("#dvCarryOutStatusChange").html("<a style=\"color:#3d9970;border-color:#3d9970;\" class=\"start-btn-one\" onclick=\"ChangeCarryoutStatus(" + storeId + ",'RUNNING')\"><i class=\"fa fa-play start-font\" aria-hidden=\"true\"></i>START</a>");
                }
...
function Logout() {
    ...
    var storeId = window.localStorage.getItem("StoreId").trim();
    if (window.localStorage.getItem("DeviceRegistrationToken") === null) {
        window.location.href = "index.html";
        window.localStorage.clear();

    }
    else {
        var token = window.localStorage.getItem("DeviceRegistrationToken").trim();
...
//   // WriteLog(GetCurrentDateTime() + " - " + "CheckNewOrder START", browser);
//    var params = getParams();
//    var storeId = 0;
//    storeId = SetStoreId();
//    if (Number(storeId) > 0) {
...
//        try {
//            WriteLog(GetCurrentDateTime() + " - " + "Searching for new orders", browser);
//            $.getJSON(url, function (data) {
//                var obj = JSON.parse(data).Rows;
//                if (data.indexOf("No order(s) found.") > -1) {
//                    WriteLog(GetCurrentDateTime() + " - " + " No new order(s) found", browser);
//                }
//                else {
...
//                        WriteLog(GetCurrentDateTime() + " - " + " Found new orders(" + orderIds + ")", browser);
//                        $('#myModal').modal();
//                        if (isDevice()) {
//                            playAudio();
//                        }
...
//                    else {
//                        console.log("2");
//                        WriteLog(GetCurrentDateTime() + " - " + " No new order(s) found(2)", browser);
//                    }
...
//        }
//        catch (e) {
//            WriteLog(GetCurrentDateTime() + " - " + " Error CheckNewOrder", browser);
//        }
//    }

//    WriteLog(GetCurrentDateTime() + " - " + "CheckNewOrder END", browser);
//}
function HideModal() {
    $(".modal").modal("hide");
    //$('#modal').modal('hide');
...
function CheckGiftCardPermission() {
    ...
    storeId = SetStoreId();
    SetMenuNavigation(storeId);
    var carryOutEnabled = window.localStorage.getItem("CarryOutEnabled").trim();
    var giftCardsEnabled = window.localStorage.getItem("GiftCardsEnabled").trim();
    var giftCardProgramEnabled = window.localStorage.getItem("GiftCardProgramEnabled").trim();
    var rewardEnabled = window.localStorage.getItem("RewardsEnabled").trim();
    //alert(rewardEnabled);
    //alert(giftCardProgramEnabled);

    if (carryOutEnabled != "" && carryOutEnabled == "True") {
        $('#aCarryout img').attr("src", "images/menu_2.png");
        $("#aCarryout").attr("href", "carryout.html?StoreId=" + storeId);
        $("#headerCarryout").css("color", "#fff");
    }
    else {
        $('#aCarryout img').attr("src", "images/menu_2-gray.png");
        $("#aCarryout").removeAttr("href");
        $("#headerCarryout").prop("onclick", null).off("click");
        $("#headerCarryout").css("color", "#504f4f");
    }

    if (giftCardsEnabled != "" && giftCardsEnabled == "True") {
        //|| giftCardProgramEnabled != "" && giftCardProgramEnabled == "True"
        $("#aGiftCard img").attr("src", "images/menu_4.png");
        $("#aGiftCard").attr("href", "giftcardsredeem.html?StoreId=" + storeId);
        $("#headerGiftCards").css("color", "#fff");

        if (giftCardProgramEnabled == "" || giftCardProgramEnabled != "True") {
            $("#divGiftCardLoad").prop("onclick", null).off("click");
            $("#divGiftCardLoad").css("color", "#504f4f");
            $("#menuRedeem").prop("onclick", null).off("click");
            $("#menuRedeem").css("color", "#504f4f");
        }
    }
    else {
        $("#aGiftCard img").attr("src", "images/menu_4-gray.png");
        $("#aGiftCard").removeAttr("href");
        $("#headerGiftCards").prop("onclick", null).off("click");
        $("#headerGiftCards").css("color", "#504f4f");
    }


    if (rewardEnabled != "" && rewardEnabled == "True") {
        $('#aReward img').attr("src", "images/menu_7.png");
        $("#aReward").attr("href", "rewards.html?StoreId=" + storeId);
        $("#headerRewards").css("color", "#fff");
    }
    else {
        $('#aReward img').attr("src", "images/menu_7-gray.png");
        $("#aReward").removeAttr("href");
        $("#headerRewards").prop("onclick", null).off("click");
        $("#headerRewards").css("color", "#504f4f");
    }
}
...
function SearchGiftCard() {
    ...
    $("#txtRedeem").css('border', noErrorClassBorder)
    $("#txtLoad").css('border', noErrorClassBorder)
    $("#txtRedeem").val("");
    $("#txtLoad").val("");
    ...
    if (storeId > 0) {
        ...
        if (cardCode != "") {
            $("#txtCardCodeSearch").css('border', noErrorClassBorder);
                ...
                $.getJSON(url, function (data) {
                    ...
                    if (data.replace(/"/g, "").indexOf("Phone is not valid.") > -1) {
                        ...
                        if (phone.trim() === '' || phone === '0') {
                            ...
                            $('#dvOuter').hide();
                            $('#dvOuterText').html("");
                            $("#txtPhoneSearch").css('border', errorClassBorder);
                        }
                        else {
                            console.log("2");
                            $("#txtPhoneSearch").css('border', noErrorClassBorder);
                        ...
                        $.each(JSON.parse(data), function (index, value) {
                            ...
                            if (value.Type == "GiftCardInfo") {
                                ...
                                $("#lblEmail").html(email);
                                $('#hdnSelectedOrderId').val(orderId);
                                $('#lblCurrentBalance').html("Current Balance: " + balanceAmount);
                                $('#lblOriginalValue').html("Original Value: " + amount);
            ...
            //$('#dvOuterText').html("");
            $('#dvInner').hide();
            $("#txtCardCodeSearch").css('border', errorClassBorder);
            //$('#dvOuter').show();
            //$('#dvOuterText').html("");
    ...
    if (storeId > 0) {
        if (cardCode != "" && amount != "" && amount != "0") {
            ...
            if (cardCode == "") {
                ...
                $("#txtCardCodeSearch").css('border', noErrorClassBorder);
                $("#txtRedeem").css('border', errorClassBorder);
            }
            $('#btnRedeemGiftCard').text("Redeem");
        }
    }
        ...
        try {
            ...
            $.getJSON(url, function (data) {
                ...
                if (data.replace(/"/g, "").indexOf("Phone is not valid.") > -1) {
                    ...
                    $('#dvOuter').hide();
                    $('#dvOuterText').html("");
                    $("#txtLoad").css('border', errorClassBorder);
                }
                else {
                    SearchGiftCard();
...
function LoadNewGiftCard() {
    ...
    $("#txtCardCode").css('border', noErrorClassBorder);
    $("#txtAmount").css('border', noErrorClassBorder);
    $("#txtEmail").css('border', noErrorClassBorder);
    $("#txtName").css('border', noErrorClassBorder);
    ...
    if (cardCode != "" && amount != "" && amount != "0") {
        ...
        if (isEmail("#txtEmail") == true) {
            ...
            try {
                ...
                $.getJSON(url, function (data) {
                    ...
                    $("#txtCardCode").css('border', noErrorClassBorder);
                    $("#txtAmount").css('border', noErrorClassBorder);
                    $("#txtPhone").css('border', noErrorClassBorder);
                    $("#txtName").css('border', noErrorClassBorder);
                    $("#txtEmail").css('border', noErrorClassBorder);
                    ...
                    if (data.indexOf("Card is already in the system.") > -1) {
                        ...
                        $("#txtCardCode").css('border', errorClassBorder);
        ...
        if (amount === "" || amount === "0") {
            ...
            $("#txtAmount").css('border', errorClassBorder);
...
function GiftCardOrdersList(pagesize, currentPage) {
    ...
    window.localStorage.setItem("GiftCardCurrentPage", currentPage);
    $("#dvOrderList").html("");
    //$("#dvItem").html("");
            ...
            $.getJSON(url, function (data) {
                ...
                if (JSON.parse(data).indexOf("No giftcard(s) found") < 0) {
                    ...
                    $.each(JSON.parse(data), function (index, value) {
                        ...
                        /*------------------Order Row-----------------------*/
                        var html = "<div class=\"order-list panel-open\" data-panel=\"left\" id='li_" + value.ID + "'  data-popup=\".popup-details\" onclick=\"OpenGiftCardDetails(" + value.ID + ");\" >";
                        /*------------------Column 1-----------------------*/
                        ...
                        if (value.ORDERSTATUSID.toLowerCase() == "new") {
                            ...
                            iconHtml += "<img class=\"giftcard-icon\" src=\"img/icons/process.png\" alt=\"\"/>";
                        }
    ...
    if (Number(storeId) > 0) {
        ...
        try {
            ...
            $.getJSON(url, function (data) {
                ...
                $('#loader_msg').html("");
                if (JSON.parse(data).indexOf("No order(s) found") < 0) {
                    window.localStorage.setItem("GiftCardAvailable", "1");
                    var count = 0;
                    $.each(JSON.parse(data), function (index, value) {
                        //   storeId = 8;
                        var orderDate = "";
                        ...
                        var html = "";
                        if (count == 0) {
                            html = "<li id=\"li_" + value.ID + "_\" class=\"active\">";
                        }
                        else {
                            html = "<li id=\"li_" + value.ID + "_\">";
                        }
                        count++;
                        //if (status == "") {
                        var htmlPickUpTime = "";
                        if (value.GIFTCARDCOUPONCODE != undefined) {
                            htmlPickUpTime += "<label class=\"label-seven\">" + value.GIFTCARDCOUPONCODE;

                            if (value.ORDERSTATUSID.toLowerCase() == "new") {
                                htmlPickUpTime += "<div class=\"icon-style\"><img  src=\"img/new.png\" alt=\"\"/></div></label>";
                            }
                            else if (value.ORDERSTATUSID.toLowerCase() == "processing") {
                                htmlPickUpTime += "<div class=\"icon-style\"><img  src=\"img/process.png\" alt=\"\"/></div></label>";
                            }
                            else if (value.ORDERSTATUSID.toLowerCase() == "shipped") {
                                htmlPickUpTime += "<div class=\"icon-style\"><img  src=\"img/shipped.png\" alt=\"\"/></div></label>";
                            }
                            else if (value.ORDERSTATUSID.toLowerCase() == "pickedup") {
                                htmlPickUpTime += "<div class=\"icon-style\"><img  src=\"img/pickup.png\" alt=\"\"/></div></label>";
                            }
                            else if (value.ORDERSTATUSID.toLowerCase() == "complete") {
                                htmlPickUpTime += "<div class=\"icon-style\"><img  src=\"img/completed.png\" alt=\"\"/></div></label>";
                            }
                        }
                        else {
                            if (value.ORDERSTATUSID.toLowerCase() == "new") {
                        ...
                        html += htmlPickUpTime + "<h2><label class=\"label-one\">#" + value.ORDERID + "</label><label class=\"label-two\">" + giftcardBalance + "</label></h2>";


                        html += "<label class=\"label-three\">" + orderDate + "</label>";
                        html += "<label class=\"label-three\">" + orderTime + "</label>";
                    ...
                    window.localStorage.setItem("GiftCardAvailable", "0");
    ...
    try {
        ...
        $.getJSON(url, function (data) {
            if (data.indexOf("Successful") > -1) {
                ...
                $("#lbl_gc_" + giftCardId).html(giftcardcode);
            }
            else {
...
function GiftCardBack() {
    ...
    if (typeof (params["StoreId"]) != "undefined") {
        ...
        if (window.localStorage.getItem("StoreId") != null)
            storeId = window.localStorage.getItem("StoreId").trim();
    }
...
function AddNewMemberID() {
    ...
    $("#txtPhone").css('border', noErrorClassBorder);
    $("#txtPoints").css('border', noErrorClassBorder);
    ...
    if (phone == "") {
        $("#txtPhone").css('border', errorClassBorder);
    ...
    if (points == "") {
        $("#txtPoints").css('border', errorClassBorder);
    ...
    if (valid == true) {
        ...
        console.log("email:" + email);
        var url = global + "/CheckCustomerExists?storeid=" + storeId + "&email=" + encodeURIComponent(email) + "&phone=" + phone;
        $.getJSON(url, function (data) {
            console.log(data);

            var dd = JSON.parse(data);
            if (dd.CustomerExists.toString().toLowerCase() == "true") {
                console.log("Member is already in the system.");
                swal({
                ...
                $.getJSON(url, function (data1) {
                    ...
                    $.each(JSON.parse(data1), function (index, value) {
                        ...
                        //console.log("ACTIONTYPE: " + value.ACTIONTYPE);
                        //console.log("REWARDMEMBERID: " + value.REWARDMEMBERID);
                        var popuphtml = "";
